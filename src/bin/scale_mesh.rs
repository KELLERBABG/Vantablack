...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    loop {
        ...
        let sock_addr = exit_rotator.get_next_socket_addr(80);
        ...
        let live_google_data = match tokio::time::timeout(Duration::from_secs(4), async {
            ...
            let mut live_buf = vec![0u8; 1024];
            let n = tokio::io::AsyncReadExt::read(&mut stream, &mut live_buf).await?;
            live_buf.truncate(n);
            Ok::<Vec<u8>, std::io::Error>(live_buf)
        }).await {
            Ok(Ok(bytes)) => {
                println!("       [LIVE WAN] Exit Node fetched {} live bytes from www.google.com:80!", bytes.len());
                bytes
            }
            _ => b"HTTP/1.1 200 OK\r\nDate: mesh-fallback\r\n\r\n".to_vec()
        };
        ...
        for line in client_received_str.lines().take(2) {
            println!("         {}", line);
        }
