...
pub fn open_to_tun(
    ...
    let e = state.current_epoch();
    match state.ingress.open(&state.key(), &state.fingerprint, e, wire) {
...
pub fn build_keepalive(hub_overlay: std::net::Ipv4Addr, client_ip: std::net::Ipv4Addr) -> Vec<u8> {
