...
3. **Lease observe:** `LeaseTable::observe_tunnel_packet` runs the re-anchor ladder (§5).
4. **Route:** IPv4 TCP → netstack (NAT + smoltcp); UDP → flow table; ICMP echo → answered in userspace.
...
## 5. Endpoint Migration: the Re-Anchor Ladder

The hub maps fingerprint → current `SocketAddr`. On mobile networks the address changes constantly (Wi-Fi ↔ LTE), and late/reordered packets from the *old* address must never flap the endpoint back. Precedence, strictly ordered:

1. **Valid handshake (signed, fresh ephemeral key) → epoch rotation.** Processed on the handshake path before any data-session replay window exists. Sets a new epoch, `V_MAX = 0`, clears the window, re-anchors unconditionally, and **evicts all per-epoch state** (UDP flows, netstack sockets, live tunnel channels) so a stale epoch's buffers can never answer. This is the reboot path: a phone that crashed mid-session returns with counter 0, which the *old* epoch's window would reject — correctly — and re-handshakes instead.
2. **Window-advance re-anchor.** A data packet from a new address may move the endpoint **only if its counter advances the replay window** (`N > V_MAX`, the replay algorithm's "newer than anything seen" branch). A late packet from the dead Wi-Fi address (in-window, not advancing) is accepted *as data* but can *never* move the endpoint.
3. **Silence fallback.** If the current endpoint has been silent ≥ 15 s, an authenticated *in-window* packet from a different address re-anchors — this rescues route-flapping clients whose counter legitimately fell behind `V_MAX`. It can never fire for a rebooted client (step 1 owns that case).

Verified by the migration-race gate: `N+1` from `IP_B`, then `N` from `IP_A` → return traffic stays locked to `IP_B`.
...
- **`ConnectivityManager.NetworkCallback`** (`onAvailable`/`onLost`): re-bind + re-`protect()` the socket on network change, signal the core to re-STUN and migrate immediately — without tearing down the TUN, the lease, or the session. The re-anchor ladder (§5) makes the address change invisible to the tunnel.
...
| M3 hardware | Wi-Fi→LTE handover mid-SSH, no session loss | pending physical devices |

Run everything: `cargo test --features vpn`. The pristine surface: `cargo test` (66 lib tests, no vpn code compiled).

---

## 10. Known Limits / Roadmap

- **M4 / iOS:** NetworkExtension port of the same Rust core (separate distribution effort).
- **mDNS relay (v2):** reflect 224.0.0.251:5353 across the tunnel for Bonjour devices.
- **LAN-visible client mode:** optional static routes on LAN devices + hub TUN (kernel NAT) instead of proxy sockets, for use cases that need the client to hold a real LAN IP.
- **IPv6 overlay:** the wire format is version-agnostic; only the IPv4 path is implemented.
