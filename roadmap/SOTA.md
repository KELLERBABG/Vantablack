...
| **Network (NAT/relay/transport)** | zero-config mesh | `NatHolePuncher::punch_hole` sends 5 `GHOST_HPUNCH_` + `port+1..5` prediction and flips `connected=true` — **not** `RFC8489/8445 ICE`. No `TURN/DERP` relay, no UPnP-PMP, no QUIC/multipath, `ContactPlan::latency()=10ms` hard-coded, `Journey` never populated. `FlowController` is token bucket, not BBR/CUBIC. | `P0` — blocks adoption |
...
| **Verification** | audit-ready | `cargo test --features vpn` 174 green, `vpn_loopback_test.sh` PASS, fuzz targets real. No `Tamarin/ProVerif` model, `TemporalIsolator` = 10 dummy `decapsulate` loops not constant-time proof, duplicate `tests/virtual_net.rs` vs `tests/common/virtual_net.rs`. | `P1` |
